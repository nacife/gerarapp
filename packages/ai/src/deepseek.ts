...
/**
 * Provider via API DeepSeek (compatível com OpenAI SDK, baseURL diferente).
 * Embedding, síntese e ilustração delegam ao mock.
 */
export class DeepSeekProvider implements AiProvider {
  ...
  async structureContent(input: StructureContentInput): Promise<StructureContentOutput> {
    const raw = await this.chat(
      'Extraia a estrutura do documento como JSON com capítulos e seções.',
      `Arquivo: ${input.filename}\n\n${input.rawText}`,
      8192,
    );
    try {
      return JSON.parse(raw) as StructureContentOutput;
    } catch {
      return { tree: [] };
    }
  }
  ...
  async generateInteractions(input: GenerateInteractionsInput): Promise<GeneratedInteraction[]> {
    ...
    const raw = await this.chat(
      `Gere interações educacionais do tipo ${types}. Retorne JSON array.`,
      `Bloco: ${input.block.contentMd}\nDensidade: ${input.density}`,
      8192,
    );
    try {
  ...
  async regenerateInteraction(input: {
    ...
    const raw = await this.chat(
      `Regenere UMA interação do tipo ${input.type}. JSON.`,
    ...
    try {
      ...
      return { type: input.type as any, payload: {}, difficulty: 'medium' } as any;
    }
  }

  async generateMemorial(input: MemorialInput): Promise<MemorialOutput> {
    const raw = await this.chat(
      'Escreva memorial descritivo para registro de software no INPI.',
      `App: ${input.title}\nCapítulos: ${input.chapterTitles.join(', ')}`,
      8192,
    );
    return {
      functionalDescription: raw,
      architectureDescription: `App ${input.title}`,
      applicationField: 'Educação',
  ...
  async tutorAnswer(input: TutorAnswerInput): Promise<TutorAnswerOutput> {
    if (input.chunks.length === 0)
      return { answer: 'Fora do escopo.', citations: [], refused: true };
    const ctx = input.chunks
      .map((c, i) => `[FONTE ${i + 1}] p.${(c.sourceRef as any)?.page ?? '?'}: ${c.contentMd}`)
      .join('\n\n');
    const raw = await this.chat(
      `Você é "${input.tutorName}", tom ${input.tone}. Use APENAS as fontes. Cite [FONTE N].`,
      `Fontes:\n${ctx}\n\nPergunta: ${input.question}`,
      2048,
    );
    const citations: { blockId: string }[] = [];
    const re = /\[FONTE\s+(\d+)\]/g;
    let m: RegExpExecArray | null;
    while ((m = re.exec(raw)) !== null) {
      const idx = parseInt(m[1]!) - 1;
      const c = input.chunks[idx];
      if (c && !citations.some((x) => x.blockId === c.blockId))
        citations.push({ blockId: c.blockId });
    }
  ...
  async generatePodcastScript(input: PodcastScriptInput): Promise<PodcastScriptOutput> {
    ...
    const raw = await this.chat(
      'Crie roteiro de podcast com 2 apresentadores (A e B). JSON.',
      `${input.appTitle} - ${input.chapterTitle}\n\n${txt}`,
      8192,
    );
    try {
