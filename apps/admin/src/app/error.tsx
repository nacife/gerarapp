    ...
    <div className="flex min-h-screen flex-col items-center justify-center px-4 py-12 text-slate-100">
      <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900/90 p-8 shadow-2xl backdrop-blur-xl">
        <div className="mb-6 flex justify-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-rose-500/20 to-orange-500/20 text-3xl border border-rose-500/30">
            🛡️
          </div>
        </div>
        ...
        <p className="mt-2 text-center text-sm text-slate-400">
          Ocorreu um erro inesperado durante a execução desta operação ou visualização administrativa.
        </p>

        <div className="mt-8 flex flex-col gap-3">
          <button
            onClick={() => reset()}
            className="w-full rounded-xl bg-gradient-to-r from-rose-500 to-amber-600 px-4 py-3 font-semibold text-white shadow-lg shadow-rose-500/20 transition hover:brightness-110 active:scale-[0.99]"
          >
            Tentar Novamente
          </button>
          ...
          <Link
            href="/painel"
            className="w-full text-center py-2 text-xs font-medium text-slate-400 hover:text-rose-400 transition"
          >
        ...
        {error?.message && (
          <div className="mt-6 border-t border-slate-800 pt-4">
            ...
            {showDetails && (
              <div className="mt-2 rounded-lg bg-slate-950 p-3 text-xs text-rose-300 font-mono break-all max-h-40 overflow-auto border border-rose-950">
                <p>{error.message}</p>
