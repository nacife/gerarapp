    ...
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-12">
      ...
      <div className="flex gap-3 text-sm">
        <span className="text-gray-500">
          ⏳ Aguardando: <b className="text-gray-200">{totalWaiting}</b>
        </span>
        <span className="text-cyan-300">
          ▶ Ativos: <b>{totalActive}</b>
        </span>
        <span className="text-red-300">
          ✕ Falhas: <b>{totalFailed}</b>
        </span>
      </div>
          ...
          <div key={name} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                ...
                {stats.completed > 0 && (
                  <button
                    onClick={() => cleanCompleted(name)}
                    className="rounded bg-white/[0.04] px-2 py-1 text-xs text-gray-500 hover:text-gray-300"
                  >
            ...
            <div className="flex gap-4 text-sm">
              <Bar
                ...
                value={stats.waiting}
                color="bg-gray-500"
                total={stats.waiting + stats.active + stats.failed + stats.completed || 1}
              />
              <Bar
                label="▶ Active"
                value={stats.active}
