...
export default function AiConfigPage() {
  ...
  async function saveKey(e: FormEvent) {
    e.preventDefault();
    const res = await apiFetch('/admin/ai-config/provider', {
      method: 'POST',
      body: { provider: selProvider, apiKey, model: selModel },
    });
  ...
  async function moveUp(idx: number) {
    ...
    await apiFetch('/admin/ai-config/fallback', {
      method: 'POST',
      body: { order, mockFallback: config.mockFallback },
    });
  ...
  async function toggleMockFallback() {
    ...
    await apiFetch('/admin/ai-config/fallback', {
      method: 'POST',
      body: { order: config.fallbackOrder, mockFallback: v },
    });
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-12">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-100">🤖 IA / LLMs — Multi-Provedor</h1>
      ...
      {msg && (
        <div
          className="rounded-lg border border-cyan-500/30 bg-cyan-500/10 p-2 text-sm text-cyan-300"
          onClick={() => setMsg(null)}
        >
          {msg}
        </div>
            ...
            return (
              <div key={key} className={`rounded-xl border ${meta.color} p-4 space-y-2`}>
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${p.hasKey ? 'bg-emerald-500/15 text-emerald-300' : 'bg-red-500/10 text-red-300'}`}
                  >
                    {p.hasKey ? '✅ Ativo' : '⬜ Off'}
                  </span>
                </div>
                <p className="text-xs text-gray-500">
                  Modelo: <span className="text-gray-300">{p.model}</span>
                </p>
                {p.keyPreview && (
                  <p className="text-xs text-gray-500">
                    Chave: <span className="font-mono text-emerald-300">{p.keyPreview}</span>{' '}
                    <span className="text-gray-600">({p.keySource})</span>
                  </p>
                )}
                <div className="flex gap-2">
                  <button
                    onClick={() => selectProvider(key)}
                    className="rounded bg-cyan-500/10 px-2 py-1 text-xs text-cyan-300 hover:bg-cyan-500/20"
                  >
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <select
                ...
                className="w-full rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-sm text-gray-200"
              >
                {PROVIDER_META[selProvider]?.models.map((m) => (
            ...
            <div>
              ...
              <input
                ...
                type="password"
                placeholder="sk-..."
                className="w-full rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-sm text-gray-200 font-mono"
              />
            </div>
          </div>
          <button
            type="submit"
            className="rounded-lg bg-cyan-500 px-4 py-2 text-sm font-semibold text-gray-950"
          >
            Salvar no Redis
          </button>
        </form>
      ...
      {config && (
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-6 space-y-4">
          ...
          <p className="text-xs text-gray-500">
            Em caso de falha, o sistema tenta o próximo provider da lista.
          </p>
          <div className="space-y-1">
            {config.fallbackOrder.map((p, i) => {
              ...
              return (
                <div
                  key={p}
                  className={`flex items-center gap-3 rounded-lg border border-white/[0.04] bg-white/[0.01] p-3 ${!pinfo?.hasKey ? 'opacity-50' : ''}`}
                >
                  ...
                  <button
                    onClick={() => moveUp(i)}
                    disabled={i === 0}
                    className="rounded bg-white/[0.04] px-2 py-0.5 text-xs text-gray-400 disabled:opacity-30"
                  >
                    ▲
                  </button>
                  <button
          ...
          <div className="flex items-center gap-3 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
            ...
            <span className="flex-1 text-sm text-amber-200">
              🤖 MockAiProvider (fallback final)
            </span>
            <button
              onClick={toggleMockFallback}
              className={`rounded-full px-3 py-0.5 text-xs font-medium transition ${config.mockFallback ? 'bg-emerald-500/15 text-emerald-300' : 'bg-red-500/10 text-red-300'}`}
            >
...
function Stat({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="rounded-lg border border-white/[0.03] p-3 text-center">
