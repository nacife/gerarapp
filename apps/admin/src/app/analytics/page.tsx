        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <StatCard
              ...
              sub={`+${data.users.today} hoje · +${data.users.thisMonth} no mês`}
              color="text-gray-200"
            />
            <StatCard
              label="Projetos"
              value={fmt(data.projects.total)}
              sub={`+${data.projects.today} hoje`}
              color="text-gray-200"
            />
            <StatCard
              label="Interações"
              value={fmt(data.interactions)}
              sub="geradas por IA"
              color="text-cyan-300"
            />
            <StatCard
              label="Matrículas"
              value={fmt(data.enrollments)}
              sub="de aprendizes"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <StatCard
              label="Créditos IA (total)"
              value={fmt(data.credits.consumedTotal)}
              sub="consumidos desde o início"
              color="text-amber-300"
            />
            <StatCard
              label="Créditos IA (30d)"
              value={fmt(data.credits.consumedThisMonth)}
              sub="consumidos este mês"
              color="text-amber-300"
            />
          </div>
          ...
          <section className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-6">
            <h2 className="mb-3 text-lg font-semibold text-gray-200">
              Top 5 Projetos (matrículas)
            </h2>
            <div className="space-y-2">
              {data.topProjects.map((p, i) => (
                <div
                  key={p.id}
                  className="flex items-center justify-between rounded-lg border border-white/[0.03] p-3 text-sm"
                >
                  <span className="text-gray-400">
                    #{i + 1} <span className="text-gray-200">{p.title}</span>{' '}
