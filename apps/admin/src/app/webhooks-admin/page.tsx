...
interface Delivery {
  ...
  createdAt: string;
  endpoint: { url: string; user: { name: string; email: string } };
}
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-12">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-100">Webhooks (Global)</h1>
      ...
      {stats && (
        <div className="flex gap-4 text-sm">
          <span className="text-gray-500">
            Endpoints: <b className="text-gray-200">{stats.totalEndpoints}</b>
          </span>
          <span className="text-gray-500">
            Entregas: <b className="text-gray-200">{stats.totalDeliveries}</b>
          </span>
          <span className="text-red-300">
            Falhas: <b>{stats.failedDeliveries}</b>
          </span>
          <span className="text-amber-300">
            Pendentes: <b>{stats.pendingDeliveries}</b>
          </span>
        </div>
      )}
      <div className="flex gap-2">
        <button
          onClick={() => setTab('all')}
          className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === 'all' ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-500'}`}
        >
          Todas ({deliveries.length})
        </button>
        <button
          onClick={() => setTab('failed')}
          className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === 'failed' ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-500'}`}
        >
      ...
      <div className="space-y-1">
        {list.map((d) => (
          <div
            key={d.id}
            className={`flex items-center justify-between rounded border p-2 text-xs ${d.status === 'failed' ? 'border-red-500/15 bg-red-500/5' : 'border-white/[0.03] bg-white/[0.01]'}`}
          >
            <div>
              ...
              <span
                className={`ml-2 font-medium ${d.status === 'succeeded' ? 'text-emerald-300' : d.status === 'failed' ? 'text-red-300' : d.status === 'pending' ? 'text-amber-300' : 'text-gray-500'}`}
              >
                {d.status}
              </span>
            ...
            {d.status === 'failed' && (
              <button
                ...
                className="rounded bg-cyan-500/10 px-2 py-0.5 text-cyan-300 hover:bg-cyan-500/20"
              >
                Reenviar
              </button>
            )}
