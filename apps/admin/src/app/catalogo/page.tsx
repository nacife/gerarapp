  ...
  async function createPalette(e: FormEvent) {
    ...
    const res = await apiFetch('/admin/catalog/palettes', {
      ...
      body: {
        key,
        name,
        colors: { light: { primary, secondary, accent }, dark: { primary, secondary, accent } },
      },
    });
    if (res.ok) {
      setShowForm(false);
    ...
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-12">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === 'palettes' ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-500 hover:text-gray-300'}`}
        >
          Paletas ({palettes.length})
        </button>
        <button
          onClick={() => setTab('templates')}
          className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === 'templates' ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-500 hover:text-gray-300'}`}
        >
          ...
          {showForm && (
            ...
            >
              <input
                ...
                placeholder="Key (ex: meu-tema)"
                className="w-full rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-sm text-gray-200 outline-none"
                required
              />
              <input
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Nome (ex: Meu Tema)"
                className="w-full rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-sm text-gray-200 outline-none"
                required
              />
              <div className="flex gap-2">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="flex gap-1">
                    {(p.colors as any)?.light?.primary && (
                      <>
                        <span
                          className="h-5 w-5 rounded"
                          style={{ background: (p.colors as any).light.primary }}
                        />
                        <span
                          className="h-5 w-5 rounded"
                          style={{ background: (p.colors as any).light.secondary }}
                        />
                        <span
                          className="h-5 w-5 rounded"
                          style={{ background: (p.colors as any).light.accent }}
                        />
                      </>
                    )}
                  </div>
