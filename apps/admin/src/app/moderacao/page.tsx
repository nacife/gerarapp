  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-12">
      ...
      <div className="space-y-2">
            ...
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-medium text-gray-200">
                  {SOURCE[c.source] ?? c.source}: {c.project.title}
                </p>
                <p className="text-xs text-gray-500">
                  {c.project.slug} · {new Date(c.createdAt).toLocaleDateString('pt-BR')}{' '}
                  {c.assignee && `· ${c.assignee.name}`}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <span
                  className={`rounded px-2 py-0.5 text-xs font-medium ${c.status === 'open' ? 'bg-red-500/10 text-red-300' : c.status === 'reviewing' ? 'bg-amber-500/10 text-amber-300' : c.status === 'resolved' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-gray-500/10 text-gray-400'}`}
                >
                  {STATUS[c.status] ?? c.status}
                </span>
                {c.status === 'open' && (
                  <button
                    onClick={() => updateStatus(c.id, 'reviewing')}
                ...
                {c.status === 'reviewing' && (
                  <>
                    <button
                      ...
                      className="rounded bg-emerald-500/10 px-2 py-1 text-xs text-emerald-300 hover:bg-emerald-500/20"
                    >
                      Resolver
                    </button>
                    <button
                      onClick={() => updateStatus(c.id, 'takedown')}
        ...
        {cases.length === 0 && (
          <p className="text-center text-gray-500 py-8">Nenhum caso de moderação.</p>
        )}
      </div>
    </main>
  );
