    ...
    <main className="mx-auto max-w-5xl space-y-6 px-6 py-12">
      ...
      {stats && (
        <div className="flex gap-4 text-sm">
          <span className="text-gray-500">
            Total: <b className="text-gray-200">{stats.total}</b>
          </span>
          <span className="text-emerald-300">
            Ativas: <b>{stats.active}</b>
          </span>
          <span className="text-red-300">
            Revogadas: <b>{stats.revoked}</b>
          </span>
          <span className="text-gray-500">
            Usuários: <b>{stats.uniqueUsers}</b>
          </span>
        </div>
      )}
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === 'list' ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-500 hover:text-gray-300'}`}
        >
          Chaves ({keys.length})
        </button>
        <button
          onClick={() => setTab('create')}
          className={`rounded-lg px-4 py-2 text-sm font-medium ${tab === 'create' ? 'bg-cyan-500/20 text-cyan-300' : 'text-gray-500 hover:text-gray-300'}`}
        >
          + Criar chave
        </button>
      </div>
          ...
          {createdKey && (
            <div className="rounded-xl border-2 border-emerald-500/30 bg-emerald-500/5 p-4 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 break-all rounded bg-zinc-950 px-3 py-2 font-mono text-sm text-emerald-300">
                  {createdKey.fullKey}
                </code>
                <button
                  onClick={() => copyToClipboard(createdKey.fullKey)}
                  className="rounded-lg bg-emerald-500/20 px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-500/30"
                >
                  📋 Copiar
                </button>
              </div>
              <p className="text-xs text-gray-500">
                Prefixo: {createdKey.prefix}*** · ID: {createdKey.id}
              </p>
            </div>
          )}
        </div>
      ...
      {tab === 'list' && (
        <div className="space-y-2">
          {keys.map((k) => (
            <div
              key={k.id}
              className={`rounded-lg border p-3 text-sm ${k.revoked ? 'border-red-500/15 bg-red-500/5 opacity-60' : 'border-white/[0.04] bg-white/[0.01]'}`}
            >
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-mono text-gray-300">{k.prefix}***</span>
                  <span
                    className={`ml-2 rounded px-1.5 py-0.5 text-xs font-medium ${k.environment === 'live' ? 'bg-emerald-500/10 text-emerald-300' : 'bg-amber-500/10 text-amber-300'}`}
                  >
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-1">
                    {(k.scopes as string[]).slice(0, 2).map((s: string) => (
                      <span
                        key={s}
                        className="rounded bg-white/[0.04] px-1.5 py-0.5 text-xs text-gray-500"
                      >
                        {s}
                      </span>
                    ))}
                    {(k.scopes as string[]).length > 2 && (
                  ...
                  {!k.revoked && (
                    <button
                      onClick={() => revoke(k.id)}
                      className="rounded bg-red-500/10 px-2 py-0.5 text-xs text-red-300 hover:bg-red-500/20"
                    >
                      Revogar
                    </button>
                  )}
