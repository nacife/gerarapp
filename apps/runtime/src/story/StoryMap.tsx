    ...
    >
      ...
      <div className="relative" style={{ height: 300 }}>
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          {map.regions
            .filter((r) => map.regions.some((r2) => r2.id !== r.id))
            .map((r, i) => {
              ...
              return (
                <line
                  ...
                  x1={`${r.position.x + 3}%`}
                  y1={`${r.position.y + 3}%`}
                  x2={`${next.position.x + 3}%`}
                  y2={`${next.position.y + 3}%`}
                  stroke={theme.border}
                  strokeWidth="1"
                  strokeDasharray="4,4"
                />
        ...
        {map.regions.map((r) => (
          ...
          >
            <span
              className={`text-2xl ${r.done ? 'opacity-100' : r.current ? 'opacity-100' : 'opacity-40'}`}
            >
              {r.emoji}
            </span>
            <span
              className={`text-xs font-medium ${r.current ? 'font-bold' : ''}`}
              style={{ color: r.current ? theme.accent : r.done ? theme.text : theme.muted }}
            >
              {r.name.slice(0, 12)}
            </span>
            {r.done && (
