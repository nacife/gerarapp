...
const SPEAKER_LABEL: Record<string, string> = {
  narrator: 'Narrador',
  host_a: 'Apresentador A',
  host_b: 'Apresentadora B',
};
...
export function AudioView({
  ...
  useEffect(() => {
    if (
      ...
      payload.media_asset_id !== '22222222-2222-2222-2222-222222222222'
    ) {
      ...
      <div className="mb-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          {hasRealAudio ? (
            <audio
              ref={audioRef}
              src={`/api/v1/public/media/${payload.media_asset_id}/stream`}
              preload="metadata"
            ...
            <SimulatedProgress
              playing={playing}
              duration={payload.duration_s ?? 60}
              theme={theme}
              onDone={() => {
                setDone(true);
                setPlaying(false);
          ...
          {SPEEDS.map((s) => (
            <button
              ...
              style={{
                ...
                color: speed === s ? theme.bg : theme.muted,
