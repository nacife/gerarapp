...
interface BattleEvent { type: 'join' | 'start' | 'score' | 'end'; data: any }
    ...
    <div className="space-y-4 rounded-xl border p-4" style={{ borderColor: theme.border, background: theme.surface }}>
      ...
      {status === 'idle' && (
        <div className="space-y-2">
          ...
          <button onClick={joinRoom} className="w-full rounded-lg px-4 py-2 text-sm font-semibold" style={{ background: theme.accent, color: theme.bg }}>Entrar na Batalha</button>
        </div>
      )}
      ...
      {status === 'waiting' && (
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h4 className="text-xs font-semibold" style={{ color: theme.text }}>Jogadores:</h4>
            {players.map(p => (
              <div key={p.id} className="text-sm flex justify-between p-2 rounded" style={{ background: theme.bg, color: theme.text }}>
                <span>{p.name}</span>
      ...
      {status === 'playing' && currentQuestion && (
        <div className="space-y-3">
          ...
          <div className="space-y-1">{currentQuestion.options.map((o, i) => <button key={i} onClick={() => handleAnswer(i === 0)} className="w-full rounded-lg border px-3 py-2 text-sm text-left" style={{ borderColor: theme.border, color: theme.text }}>{o}</button>)}</div>
          {msg && <p className="text-xs font-bold text-center" style={{ color: msg.includes('✅') ? '#4ade80' : '#f87171' }}>{msg}</p>}
          
          <div className="mt-4 pt-4 border-t" style={{ borderColor: theme.border }}>
            <h4 className="text-xs font-semibold mb-2" style={{ color: theme.text }}>Placar:</h4>
            {players.map(p => (
              <div key={p.id} className="text-sm flex justify-between" style={{ color: theme.text }}>
                <span>{p.name}</span>
