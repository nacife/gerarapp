...
export default function AssistedFilingPage({
  params,
}: {
  params: { id: string; filingId: string };
}) {
  ...
  async function sendPoa() {
    ...
    const urlRes = await apiFetch<{ uploadUrl: string }>(
      `/inpi/filings/${filingId}/poa/upload-url`,
      {
        method: 'POST',
      },
    );
  ...
  async function revoke() {
    if (
      !window.confirm(
        'Revogar a procuração cancela o Registro Assistido (sem custo de GRU). Continuar?',
      )
    )
      return;
    setBusy(true);
    ...
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-6 py-12">
      <div>
        ...
        <p className="text-sm text-slate-500">
          ...
          <span className="text-slate-300">{STATUS_LABEL[filing.status]}</span>
        </p>
      </div>
        ...
        >
          ...
          {pricing && (
            <p className="text-xs text-slate-500">
              Honorários {money(pricing.serviceFeeCents)} + GRU código 730{' '}
              {money(pricing.gruFeeCents)} = total {money(pricing.totalCents)} (cobrados
              separadamente).
            </p>
          )}
          <div className="flex gap-4 text-sm">
            <label className="flex items-center gap-1.5">
              <input
                type="radio"
                checked={holderType === 'pf'}
                onChange={() => setHolderType('pf')}
              />
              Pessoa física
            </label>
            <label className="flex items-center gap-1.5">
              <input
                type="radio"
                checked={holderType === 'pj'}
                onChange={() => setHolderType('pj')}
              />
              Pessoa jurídica
            </label>
          </div>
            ...
            {authors.map((a, i) => (
              <div key={i} className="flex gap-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAuthors((xs) =>
                      xs.map((x, j) => (j === i ? { ...x, cpf: e.target.value } : x)),
                    )
                  }
                  placeholder="CPF"
          ...
          <p className="text-xs text-amber-300">
            ⚠ Se o conteúdo foi produzido por terceiros ou funcionários, guarde o contrato de
            cessão/trabalho — a veracidade dos dados é responsabilidade legal do requerente.
          </p>
          <button
            disabled={busy}
        ...
        <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-lg bg-sky-500/20 px-4 py-2 text-sm text-sky-300"
                >
                  Sim, eu tenho
                </button>
                <button
                  onClick={() => setHasCert(false)}
                  className="flex-1 rounded-lg border border-slate-800 px-4 py-2 text-sm"
                >
                  Ainda não
                </button>
              </div>
          ...
          {hasCert === false && (
            <div className="space-y-2 rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-100">
              <p>
                Sem um certificado ICP-Brasil você não consegue assinar a procuração — o e-Software
                só aceita assinatura PAdES com e-CPF/e-CNPJ emitido por uma Autoridade Certificadora
                credenciada.
              </p>
              <p>
                Emita o seu junto a uma AC credenciada (ex.: Serasa, Certisign, Valid) e volte aqui.
              </p>
              <p className="text-xs text-amber-300">
                ⚠ Assinaturas eletrônicas avançadas (de plataformas como DocuSign/Clicksign) não são
                aceitas.
              </p>
              <button
                onClick={() => setHasCert(null)}
                className="text-xs text-sky-400 hover:underline"
              >
                ← voltar
              </button>
          ...
          {hasCert === true && (
            <div className="space-y-3">
              <p className="text-sm text-slate-400">
                Baixe o modelo de procuração, assine digitalmente (PAdES) com seu certificado e
              ...
              <div className="flex gap-3 text-sm">
                <label className="flex items-center gap-1.5">
                  <input
                    type="radio"
                    checked={signerDocType === 'e-cpf'}
                    onChange={() => setSignerDocType('e-cpf')}
                  />
                  e-CPF
                </label>
                <label className="flex items-center gap-1.5">
                  <input
                    type="radio"
                    checked={signerDocType === 'e-cnpj'}
                    onChange={() => setSignerDocType('e-cnpj')}
                  />
                  e-CNPJ
                </label>
              </div>
