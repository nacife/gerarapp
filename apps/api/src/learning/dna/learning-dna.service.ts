...
export class LearningDnaService {
  async computeProfile(enrollmentId: string, learnerId: string): Promise<DnaProfile | null> {
    ...
    const events = await prisma.learningEvent.findMany({
      ...
      select: {
        detail: true,
        occurredAt: true,
        interaction: { select: { type: true, contentBlockId: true } },
      },
    });
    ...
    const blockIds = [
      ...new Set(events.map((e) => e.interaction?.contentBlockId).filter(Boolean)),
    ] as string[];
    const blocks =
      blockIds.length > 0
    ...
    const topicCorrect = new Map<string, number>(),
      topicTotal = new Map<string, number>();
    ...
    const topics: TopicRetention[] = [...topicTotal.entries()]
      ...
      .map(([topic, total]) => ({
        topic,
        retention: (topicCorrect.get(topic) ?? 0) / total,
        events: total,
      }))
    ...
    const hourCorrect = new Map<number, number>(),
      hourTotal = new Map<number, number>();
    for (const ev of events) {
      const h = new Date(ev.occurredAt).getHours();
      hourTotal.set(h, (hourTotal.get(h) ?? 0) + 1);
      if ((ev.detail as any)?.correct === true) hourCorrect.set(h, (hourCorrect.get(h) ?? 0) + 1);
    }
    let bestHour = 9,
      bestRate = 0;
    ...
    const typeCorrect = new Map<string, number>(),
      typeTotal = new Map<string, number>();
    for (const ev of events) {
    ...
    const completedBlocks = await prisma.learnerProgress.count({
  ...
  async getCreatorAggregate(projectId: string) {
    ...
    if (profiles.length === 0)
      return { learnerCount: 0, commonWeaknesses: [], bestHours: [], avgCompletion: 0 };
    ...
    const commonWeaknesses = [...topicRetentions.entries()]
      .map(([topic, rets]) => ({
        topic,
        avgRetention: Math.round((rets.reduce((a, b) => a + b, 0) / rets.length) * 100),
      }))
    ...
    const avgCompletion = Math.round(
      profiles.reduce((s, p) => s + p.completionPercent, 0) / profiles.length,
    );
